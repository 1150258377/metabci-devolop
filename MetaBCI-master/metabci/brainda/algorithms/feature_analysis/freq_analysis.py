...
# Added utility function for DREAMER analysis
import numpy as _np
from scipy import signal as _signal

def bandpower(data, fs, fmin, fmax):
    """Compute average power in the specified frequency band.
    ...
    out = _np.zeros(n_channels)
    for ch in range(n_channels):
        f, pxx = _signal.welch(data[ch], fs=fs, nperseg=min(256, data.shape[1]))
        idx = _np.logical_and(f >= fmin, f <= fmax)
        out[ch] = _np.mean(pxx[idx])
    return out
